...
function Gestual() {
  ...
  function captureVertical() {
    ...
    debugCanvas = document.getElementById("canvas1"); 													// recupera o elemento canvas denominado no html como canvas1
    ctxDebugCanvas = debugCanvas.getContext("2d");																// criar um contexto baseado no elemento canvas selecionado
    imagem = document.getElementById("canvasCam");													// recupera a imagem
    ctxDebugCanvas.drawImage(imagem, 0, 0);																		// cola a imagem no contexto canvas
    codeImagem3 = ctxDebugCanvas.getImageData(0, 0, debugCanvas.width, debugCanvas.height);			// captura a codificação (pixels) da imagem para ser trabalhada
    ...
    prim = 0;
    ult = 0;
    for (let k = 0; k < codeImagem3.data.length; k += debugCanvas.width * 4) {
      for (let c = 0; c < debugCanvas.width * 4; c += 4) {
        codeImagem3.data[c + k + 3] = 0;
        if (Math.abs(codeImagem3.data[c + k] - codeImagem3.data[c + k + debugCanvas.width * 4 * 1]) > 5 & codeImagem3.data[c + k] > 150 & codeImagem3.data[c + k + 2] < 150 & codeImagem3.data[c + k + 1] < 150)			// se ocorrer contraste
        {
          qC++;
        }
      }
      ...
      if (qC > 8 & qC < 35) { // 1 e 25
        for (c = 0; c < debugCanvas.width * 4; c += 4) {
          codeImagem3.data[c + k + 3] = 0;
          if (Math.abs(codeImagem3.data[c + k] - codeImagem3.data[c + k + debugCanvas.width * 4 * 1]) > 5 & codeImagem3.data[c + k] > 150 & codeImagem3.data[c + k + 2] < 150 & codeImagem3.data[c + k + 1] < 150)			// se ocorrer contraste
  ...
  function captureHorizontal() {
    ...
    prim = 0;
    ult = 0;
    for (let k = 0; k < debugCanvas.width * 4; k += 4) {
      for (let c = 0; c < codeImagem3.data.length; c += debugCanvas.width * 4) {
        codeImagem3.data[c + k + 3] = 0;
        if (Math.abs(codeImagem3.data[c + k] - codeImagem3.data[c + k + debugCanvas.width * 4]) > 5 & codeImagem3.data[c + k] > 150 & codeImagem3.data[c + k + 2] < 150 & codeImagem3.data[c + k + 1] < 150)			// se ocorrer contraste
        {
          qC++;
        }
      }
  ...
  /** @type {HTMLCanvasElement} */
  const canvasElement = document.querySelector('#canvasCam');
  ...
  navigator.mediaDevices.getUserMedia({ video: true })
    ...
    .catch((error) => {
      console.log('Camera did not work.', error);
    });
